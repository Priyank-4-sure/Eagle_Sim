	...
	void FixedUpdate()
{
    if (animator && !dead)
    {
        ...
        if (inputDirection != Vector3.zero && !isAttacking)
        {
            ...
            transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, rotationDegreePerSecond * Time.deltaTime);
        }

        // Apply movement
        float speedOut = isAttacking ? 0 : inputDirection.magnitude;
        Vector3 movement = transform.forward * speedOut * walkspeed;
        movement.y = GetComponent<Rigidbody>().linearVelocity.y; // Retain vertical velocity
